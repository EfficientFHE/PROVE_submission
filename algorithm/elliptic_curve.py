...
from algorithm import big_integer
from algorithm import finite_field as finite_field_lib
...
class EllipticCurveCoordinateSystem(ABC, Generic[T]):
  ...
  @abstractmethod
  def generate_formal_coordinates(self, coordinates: List, zero: bool) -> List[T]:
    pass
...
class ECCSWeierstrass(EllipticCurveCoordinateSystem[FieldEle]):
  ...
  def __init__(
      self, config: Dict[str, Union[int, List[int]]]
  ) -> None:
...
class ECCSTwistedEdwardsExtended(ECCSWeierstrass):
  ...
  def __init__(self, config: Dict[str, Union[int, List[int]]]) -> None:
    ...
    self.A = self.ff_zero.copy(config['MA'])
    self.B = self.ff_zero.copy(config['MB'])
  ...
  def untwist(self, point_a: List[FieldEle]):
    ...
    x = (xm / self.B) + (self.A / (three * self.B))
    y = ym / self.B
    #print("uw", x, y)
    return [x, y]
  ...
  def convert_to_twisted_edwards_affine(self, point_a: ECPoint[FieldEle]) -> ECPoint[FieldEle]:
    new_point = point_a.copy()
    if new_point.coordinates is not None:
      new_point.coordinates.clear()
      x, y, z = point_a[:3]
  ...
  def twist_int_coordinates(self, coordinates: List[int]) -> List[int]:
    ff_coordinates = [self.ff_zero.copy(coordinate) for coordinate in coordinates]
    ff_coordinates = self.twist(ff_coordinates)
    if(ff_coordinates is None):
      return [0, 1, 1, 0] # zero
    ff_coordinates.append(self.ff_one)
    ff_coordinates.append(ff_coordinates[0] * ff_coordinates[1])
    return [int(coord) for coord in ff_coordinates]
  ...
  # def untwist_projective(self, point_a):
  #   xt, yt, zt = point_a
  #   zt = zt * self.t
  #   yt = yt * self.t
  #   print("pt", xt / zt, yt / zt)
  #   # Convert to montgomery
  #   # Comon denominator
  #   zm = (self.ff_one - (yt * zt)) * (xt * zt)
  #   xm = (self.ff_one + yt * zt) * xt * zt
  #   ym = (self.ff_one + yt * zt * zt)
  #   print("pm", xm/zm, ym/zm)
  #   # Convert to weierstrass
  #   three = self.ff_zero.copy(3)
  #   z = zm * three * self.B
  #   x = (three * xm + self.A)
