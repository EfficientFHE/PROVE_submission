...
BASE = 32
BASE_TYPE = jnp.uint32  # this type must match the BASE, i.e. jnp.uint<BASE>
U16_MASK = 0xFFFF
U32_MASK = 0xFFFFFFFF
...
def int_to_array(
    python_int, base=BASE, dtype=jnp.uint32, array_size=U32_CHUNK_NUM
):
...
def int_list_to_array(int_list, base=BASE, array_size=U32_CHUNK_NUM):
...
def int_point_to_jax_point_pack(
    coordinates: List[int], base=BASE, array_size=U32_CHUNK_NUM
):
  result = []
...
def int_point_batch_to_jax_point_pack(
    points: List[List[int]], base=BASE, array_size=U32_CHUNK_NUM
):
  result = []
...
def rns_coefficients_precompute(
  ...
  rns_mat = np.array(
      icrt_factors_byteshifted_modq_rns, dtype=np.uint32
  ...
  sqe_mat = np.array(shifted_quotient_estimations, dtype=np.uint32)

  cor_mat = np.array(
      [to_rns(-overall_modulus % q, overall_moduli)], dtype=np.uint32
  )
...
num_residues_for_q =  (int(MODULUS_377_INT).bit_length() + moduli_precision - 1) // moduli_precision
NUM_MODULI = 25
extra_bit_to_avoid_addition_overflow = 40
M = (MODULUS_377_INT * 256 * NUM_MODULI * 4 * 2 * extra_bit_to_avoid_addition_overflow)**2 * 2
# ((MODULUS_377_INT * 255 * num_bytes * num_residues_for_q)**2) * extra_bit_to_avoid_addition_overflow * 2
...
MODULUS_377_INT_CHUNK = int_to_precomputed_array(
    MODULUS_377_INT, base=BASE, array_size=U32_CHUNK_NUM
)
MU_377_INT_CHUNK = int_to_precomputed_array(
    MU_377_INT, base=BASE, array_size=U32_CHUNK_NUM
)
TWIST_D_INT_CHUNK = int_to_precomputed_array(
    TWIST_D_INT, base=BASE, array_size=U32_EXT_CHUNK_NUM
)
MODULUS_377_S16_INT_CHUNK = int_to_precomputed_array(
    MODULUS_377_S16_INT, base=BASE, array_size=U32_EXT_CHUNK_NUM
)
RNS_PRECISION = 32
...
MODULI_SUB_ZERO_CHECK = tuple([
    ((256*NUM_MODULI *4* 2*MODULUS_377_INT) - 2**RNS_PRECISION) % m
    # SUB_MODULI_CONSTANT % m
    for m in MODULI
])
